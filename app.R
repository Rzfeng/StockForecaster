...
# User interface ----
ui <- fluidPage(
  titlePanel(title=div("Stock Forecaster"), windowTitle = "Stock Visualizer"),
  
  sidebarLayout(
    sidebarPanel(
      em(helpText("Richard Feng")),
      br(),
      p(strong("Select a Stock ticker to decompose and forecast: ")),
      #p("Examples: 'AAPL'(Apple), 'B'(Barnes Group), 'GE' (General Electric), 'JPM'(JP Morgan)")),
      
      textInput("symb", "Ticker Symbol:", "MSFT"),
      ...
      helpText("DISCLAIMER: Past stock performance is not indicative of future price action. 
               This site if for educational purposes only. Richard Feng does not accept any liability for your use of this information.")
        ...
        tabPanel("FAQ", 
                 h4("Should I place trades/investments based off these forecasts?"),
                    p("NO. This site has been designed for informational and educational purposes 
                      ...
                      note that past stock performance is not indicative of future price action. Richard Feng does not guarantee the validity or accuracy of any 
